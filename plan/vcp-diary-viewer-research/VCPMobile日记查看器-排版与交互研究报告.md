...
> [!IMPORTANT]
> **文档状态：视觉研究附件，不是施工 SSOT。** 2026-08-12 的源码审计确认：真实管理模型是“文件夹 → memo 文件 → 整文件内容”，而非本文所述“日期文件 → 时间戳条目”；管理 API 还需要 Basic 管理员凭据，单文件 GET 也没有已证实的 1 MiB 上限。当前里程碑包含可信 HTML 阅读、编辑/重命名、创建/批量管理与 LightMemo 语义搜索，因此本文“排除编辑”和“只读查看器”的边界同样失效。请从 [README.md](./README.md) 进入当前 `01`—`06` 施工文档；本文的字号、行高和外部案例只作候选视觉素材，须服从新文档的事实与 UI 宪法。
