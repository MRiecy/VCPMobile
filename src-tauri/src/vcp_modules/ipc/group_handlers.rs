...
use crate::vcp_modules::group_orchestrator::{determine_naturerandom_speakers, assemble_context};
use crate::vcp_modules::chat_manager::{ChatMessage, save_chat_history};
use crate::vcp_modules::vcp_client::{ActiveRequests, perform_vcp_request, VcpRequestPayload};
...
pub async fn handle_group_chat_message(
    ...
    println!("[GroupHandlers] handle_group_chat_message invoked for group: {}", payload.group_id);

    // 1. 加载群组配置
    let group_config = read_group_config(app_handle.clone(), group_state.clone(), payload.group_id.clone()).await?;
    
    // 2. 加载成员配置
    let mut active_member_configs = Vec::new();
    for member_id in &group_config.members {
        if let Ok(cfg) = read_agent_config(app_handle.clone(), agent_state.clone(), member_id.clone(), Some(false)).await {
    ...
    let speakers = if group_config.mode == "sequential" {
        ...
        println!("[GroupHandlers] Mode {} not implemented, ignoring.", group_config.mode);
    ...
    for speaker in speakers {
        ...
        let message_id = format!("assistant_{}_{}", agent_id, SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis());
        ...
        match perform_vcp_request(&app_handle, active_requests.0.clone(), final_payload).await {
            Ok((res, is_aborted)) => {
                if let Some(full_content) = res["fullContent"].as_str() {
                    ...
                    println!("[GroupHandlers] Breakpoint saved for agent: {}. Aborted: {}", agent_id, is_aborted);
                }
            ...
            Err(e) => {
                eprintln!("[GroupHandlers] Error during agent {} response: {}", agent_id, e);
