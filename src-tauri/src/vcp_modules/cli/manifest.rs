...
环境：Shell 固定为 Alpine Linux(musl) 的 /bin/bash -lc；默认 cwd=/workspace；guest 模拟 root 仅用于隔离 rootfs 和 apk，不拥有 Android root；不支持 sudo/apt/systemctl/GUI/adb。基线包含 GNU 常用文件命令、grep/sed/awk/find/diff/patch、tar/zip、curl/wget、git/ssh、jq、python3/pip/apk；其他命令先用 command -v 检查。支持管道、重定向、&& 和多行 Bash。每次 action=run 启动独立 Bash Job。工作区文件和 apk 安装结果持久；cd/export/alias 不跨 run 保留。canonical Skill catalog 永不挂载进 Bash；list_skills/read_skill 只发现和阅读，materialize_skill 才把经 hash 校验的副本放入 /workspace/.vcp-skills。该副本可被 guest 修改但不会写回 catalog。仅在 Agent 路由为 localLoopback 时，river=text、river=last:N、river=full 与 river=semantic:N 会把已过滤上下文写为本次 attempt 的 JSON 快照，路径在 VCP_RIVER_CONTEXT_FILE；full 的 artifacts[].guest_path 指向有界附件副本（最多16个、单个64MiB、总计256MiB），omissions 说明未物化原因；semantic 查询取自本次工具请求中除 tool_name/action/ink/river/vref/archery 外的非空参数值（合计最多2000 UTF-8字节），使用设备内离线多语言向量选择 N 条，模型或缓存不可用时在 JSON 与 ToolResult 中明确回退最近 N 条。guest 对上下文或附件副本的修改不会写回聊天记录或 canonical CAS。vcpPlugin 路由的 river/vref 以及 localLoopback 的 vref:N 在远端物化或本机知识授权尚未建立时，都会于命令启动前返回 unsupported_mode。
