...
`bootstrapApp()` 在普通主窗口模式下调用 `lifecycleStore.bootstrap()`；在划词助手悬浮窗模式下（`isAssistant === true`）跳过权限检查、核心就绪等待与预加载，仅初始化主题与设置，直接进入 `READY`。
...
App.vue onMounted()
    ...
    │    │     并发: Settings + AgentsAndGroups + AvatarPreload    │
...
| `vcp-floating-ball-click` | Kotlin 悬浮球 | `src/App.vue` 第 384 行 | `evaluateJavascript` 注入 | 点击悬浮球打开划词助手窗口 |
...
*最后更新：2026-07-03 | VCP Mobile v1.1.3*
