...
| 渲染帧率 | 受 innerHTML 开销限制 | 后端固定约30Hz，前端对齐下一次 VSync |
...
graph TB
    ...
    subgraph VueFrontend["Vue 3 前端（UI Layer）"]
        StreamStore["chatStreamStore<br/>rAF 绘制前原子合并"]
...
| 03 | [AstMutation 指令集与稀疏 IPC 协议](03_AstMutation指令集与稀疏IPC协议.md) | `ast_diff.rs` + `chat.ts` | 10 种突变指令详解（含 PatchCode）、TailFrame 帧信封、AuroraUpdate 稀疏序列化、IPC 生命周期 |
...
| 05 | [MessageRenderer 集成与 rAF 绘制对齐](05_MessageRenderer集成与rAF渲染节流.md) | `MessageRenderer.vue` + `chatStreamStore.ts` | 后端固定33ms门禁、前端 rAF 原子合并、mergeTailFrame、Epoch 追踪、错误恢复、调试体系 |
...
| **rAF Alignment** | requestAnimationFrame 绘制对齐 | 合并下一次 VSync 前到达的 Aurora 字段；不设置第二个时间门禁 |
...
| `vcp_client.rs` ([09_VCP客户端](../09_VCP请求客户端.md)) | **调用者** | 固定33ms合并 SSE chunk，并通过 `prepare_delta_update → send → commit_delivery` 发送 Aurora |
...
| `chatStreamStore.ts` | **绘制对齐层** | 在下一次 rAF 前原子合并 Aurora 字段；频率上限由 Rust 固定33ms门禁负责 |
