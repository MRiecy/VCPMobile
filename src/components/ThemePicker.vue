...
import { useThemeStore, type ThemeInfo } from '../stores/theme';
...
<template>
  <div :class="props.layout === 'horizontal' ? 'flex overflow-x-auto space-x-4 pb-4 snap-x' : 'grid grid-cols-2 gap-4'">
    <div
      ...
      class="relative flex-shrink-0 cursor-pointer rounded-xl overflow-hidden border-2 transition-all group"
      :class="[
    ...
    >
      ...
      >
        <div v-if="!thumbnails[theme.fileName]" class="w-full h-full flex items-center justify-center bg-gray-200 dark:bg-gray-800">
      ...
      <!-- Selection Indicator -->
      <div v-if="themeStore.currentTheme === theme.fileName" class="absolute top-2 left-2 bg-primary text-white p-1 rounded-full shadow-sm z-10 flex items-center justify-center">
