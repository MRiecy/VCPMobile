...
import type { SyncTreeNode } from '../services/syncService';
...
<template>
  <div class="sync-tree-node">
    <div 
      class="flex items-center py-2 pr-2 hover:bg-white/5 rounded-lg transition-colors cursor-pointer select-none"
      :style="{ paddingLeft: `${depth * 1.2 + 0.5}rem` }"
      @click="toggleExpand"
    >
      <!-- Expand/Collapse Icon -->
      <div class="w-6 h-6 flex-center shrink-0 opacity-50 mr-1">
        <svg v-if="!isFile" class="transition-transform duration-200" :class="{ 'rotate-90': isExpanded }" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
      ...
      <div class="flex flex-col min-w-0 flex-1">
        <div class="flex items-center gap-1.5 min-w-0">
          ...
          <template v-if="isFile && getFileStatus">
            <div :class="['flex items-center gap-0.5 shrink-0 px-1 rounded bg-white/5 border border-white/5', getFileStatus(node.path).color]">
        ...
        <span v-if="!isFile" class="text-[10px] opacity-40">{{ childFiles.length }} 项 · {{ formatBytes(node.sizeBytes) }}</span>
