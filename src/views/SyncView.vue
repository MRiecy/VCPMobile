...
import { syncService, type SyncTreeNode } from '../services/syncService';
...
const startSync = async () => {
  ...
  try {
    ...
    await syncService.startSync(filesToDownload, (current, total, file) => {
...
<template>
  <div class="sync-view h-full flex flex-col bg-secondary-bg/95 text-primary-text relative">
    <!-- Header -->
    <header class="p-4 flex items-center justify-between border-b border-white/10 pt-[calc(var(--vcp-safe-top,24px)+20px)] pb-6 shrink-0 z-10 bg-secondary-bg/80 backdrop-blur-md">
      <div class="flex items-center gap-3">
        <button @click="emit('close')" class="p-2 bg-white/5 rounded-full active:scale-90 transition-all">
    ...
    <!-- 底部同步控制栏 -->
    <div v-if="fileTree" class="absolute bottom-0 left-0 right-0 p-5 border-t border-white/10 bg-secondary-bg/95 backdrop-blur-xl pb-[calc(env(safe-area-inset-bottom,20px)+20px)] z-20">
      ...
      <!-- 防止同步时误触的透明遮罩 -->
      <div v-if="isSyncing" class="fixed inset-0 z-50 bg-transparent"></div>
    </div>
  </div>
</template>
