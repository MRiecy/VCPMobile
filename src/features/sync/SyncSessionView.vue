...
<template>
  <SlidePage :is-open="store.isOpen" :z-index="props.zIndex">
    <div
      class="fixed inset-0 flex flex-col bg-[#0a0f14] text-white overflow-hidden"
      :class="{ 'pointer-events-none': !store.isOpen }"
    >
      <!-- 顶部栏 -->
      <div
        class="flex items-center justify-between px-4 pt-[calc(env(safe-area-inset-top)+8px)] pb-3"
      >
      ...
      >
        ...
        <div class="w-full shrink-0 snap-center flex flex-col overflow-hidden">
          ...
          >
            ...
            <div
              class="text-[11px] text-white/30 text-center mb-8 leading-relaxed"
            >
              与桌面端进行数据全量比对与同步<br />
              包括会话主题、历史消息及附件
            </div>
            ...
            <div class="w-full max-w-xs mt-8 border-t border-white/5 pt-4">
              <div
                class="text-[9px] font-bold uppercase tracking-widest text-white/20 mb-3 text-left"
              >
                高级设置
              </div>
          ...
          <template v-else>
            ...
            <div class="flex-1 px-4 overflow-hidden flex flex-col min-h-0">
              <div
                ref="logContainer"
                class="bg-black/40 rounded-lg p-3 font-mono text-[10px] leading-relaxed flex-1 overflow-y-auto no-rubber-band flex flex-col min-h-0"
              >
                <div
                  v-if="store.logs.length === 0"
                  class="text-white/20 italic"
                >
                  等待连接...
                </div>
                <template v-else>
      ...
      <!-- 底部工具栏 -->
      <div
        class="flex items-center justify-between px-4 py-2 border-t border-white/5"
      >
        ...
        <button
          v-if="store.logs.length > 0 && store.activeTab === 'live'"
          @click="store.copyLogs()"
      ...
      >
        <div class="pb-8 text-center">
          <div
            class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-black/60 text-white/90 text-xs font-bold tracking-wider"
          >
            <div
