...
export const VCP_CLI_KNOWLEDGE_CATALOG_COMMAND =
  "get_vcp_mobile_cli_knowledge_catalog";
export const VCP_CLI_KNOWLEDGE_IMPORT_INSPECT_COMMAND =
  "inspect_vcp_mobile_cli_knowledge_import";
export const VCP_CLI_KNOWLEDGE_IMPORT_COMMIT_COMMAND =
  "commit_vcp_mobile_cli_knowledge_import";
export const VCP_CLI_KNOWLEDGE_IMPORT_DISCARD_COMMAND =
  "discard_vcp_mobile_cli_knowledge_import";
export const VCP_CLI_KNOWLEDGE_REVOKE_COMMAND =
...
export type VcpCliKnowledgeIndexStatus = "indexing" | "ready" | "failed";

export interface VcpCliKnowledgeSource {
  source_id: string;
  display_name: string;
  mime_type: string;
  size_bytes: number;
  source_sha256: string;
  index_status: VcpCliKnowledgeIndexStatus;
  failure_code?: string;
  index_text_truncated: boolean;
  chunk_count: number;
  granted_at_ms: number;
}

export interface VcpCliKnowledgeCatalog {
  schema_version: number;
  catalog_generation: number;
  used_bytes: number;
  limit_bytes: number;
  pending_used_bytes: number;
  pending_limit_bytes: number;
  active_source_count: number;
  active_source_limit: number;
  pending_candidate_count: number;
  pending_candidate_limit: number;
  sources: VcpCliKnowledgeSource[];
}

export interface VcpCliKnowledgeImportCandidate {
  token: string;
  candidate_sha256: string;
  catalog_generation: number;
  display_name: string;
...
export type VcpCliKnowledgeInspectResponse =
  | {
      operation_id: string;
      status: "cancelled";
      candidate?: never;
  ...
  | {
      operation_id: string;
      status: "candidate";
      candidate: VcpCliKnowledgeImportCandidate;
    };

export interface VcpCliKnowledgeInspectRequest {
  operation_id: string;
}

export interface VcpCliKnowledgeCommitRequest {
  operation_id: string;
  token: string;
  candidate_sha256: string;
...
export interface VcpCliKnowledgeDiscardRequest {
  operation_id: string;
  token: string;
}

export interface VcpCliKnowledgeRevokeRequest {
  operation_id: string;
  source_id: string;
  expected_catalog_generation: number;
}

export interface VcpCliKnowledgeCommitResponse {
  operation_id: string;
  catalog_generation: number;
  replayed: boolean;
  source: VcpCliKnowledgeSource;
}

export interface VcpCliKnowledgeDiscardResponse {
  operation_id: string;
  replayed: boolean;
}

export interface VcpCliKnowledgeRevokeResponse {
  operation_id: string;
  catalog_generation: number;
  replayed: boolean;
  source_id: string;
...
export const useVcpCliStore = defineStore("vcpCli", () => {
  ...
  const knowledgeCatalog = ref<VcpCliKnowledgeCatalog | null>(null);
  const knowledgeLoading = ref(false);
  const knowledgeError = ref<VcpCliUiError | null>(null);
  const knowledgeImportCandidate = ref<VcpCliKnowledgeImportCandidate | null>(
    null,
  );
  const knowledgeMutationBusy = ref(false);
  const knowledgeMutationError = ref<VcpCliUiError | null>(null);
  const knowledgeMutationNotice = ref("");
  const pendingKnowledgeInspectOperationId = ref<string | null>(null);
  const pendingKnowledgeCommitOperationId = ref<string | null>(null);
  const pendingKnowledgeDiscardOperationId = ref<string | null>(null);
  const pendingKnowledgeRevoke = ref<{
    operationId: string;
    sourceId: string;
    expectedCatalogGeneration: number;
  } | null>(null);
  let knowledgeCatalogGeneration = 0;
  let knowledgeMutationGeneration = 0;
  let knowledgeGenerationFloor = 0;
  ...
  function closeView(): void {
    ...
    runBusy.value = false;
    knowledgeCatalogGeneration += 1;
    knowledgeMutationGeneration += 1;
    knowledgeLoading.value = false;
    knowledgeMutationBusy.value = false;
  }
  ...
  async function loadKnowledgeCatalog(): Promise<void> {
    if (!viewVisible.value) return;
    const view = viewGeneration.value;
    const owner = ++knowledgeCatalogGeneration;
    knowledgeLoading.value = true;
    knowledgeError.value = null;
    try {
      const catalog = await invoke<VcpCliKnowledgeCatalog>(
        VCP_CLI_KNOWLEDGE_CATALOG_COMMAND,
      );
      if (!isCurrentView(view) || owner !== knowledgeCatalogGeneration) return;
      if (
        catalog.catalog_generation < knowledgeGenerationFloor ||
        (knowledgeCatalog.value &&
          catalog.catalog_generation <
            knowledgeCatalog.value.catalog_generation)
      ) {
        knowledgeError.value = {
          code: "knowledge_generation_stale",
          message: "本机知识 catalog 代际落后，请重试读取。",
        };
        return;
      }
      knowledgeCatalog.value = catalog;
      const pendingRevoke = pendingKnowledgeRevoke.value;
      if (
        pendingRevoke &&
        (pendingRevoke.expectedCatalogGeneration !==
          catalog.catalog_generation ||
          !catalog.sources.some(
            (source) => source.source_id === pendingRevoke.sourceId,
          ))
      ) {
        pendingKnowledgeRevoke.value = null;
      }
    } catch (error) {
      if (!isCurrentView(view) || owner !== knowledgeCatalogGeneration) return;
      knowledgeError.value = {
        code: "knowledge_catalog_error",
        message: describeError(error),
      };
    } finally {
      if (isCurrentView(view) && owner === knowledgeCatalogGeneration) {
        knowledgeLoading.value = false;
      }
    }
  }

  async function inspectKnowledgeImport(): Promise<void> {
    if (
      !viewVisible.value ||
      knowledgeMutationBusy.value ||
      knowledgeImportCandidate.value ||
      pendingKnowledgeCommitOperationId.value ||
      pendingKnowledgeDiscardOperationId.value ||
      pendingKnowledgeRevoke.value
    ) {
      return;
    }
    const mutation = ++knowledgeMutationGeneration;
    const view = viewGeneration.value;
    const operationId =
      pendingKnowledgeInspectOperationId.value ??
      newOperationId("knowledge-inspect");
    pendingKnowledgeInspectOperationId.value = operationId;
    knowledgeMutationBusy.value = true;
    knowledgeMutationError.value = null;
    knowledgeMutationNotice.value = "";
    try {
      const response = await invoke<VcpCliKnowledgeInspectResponse>(
        VCP_CLI_KNOWLEDGE_IMPORT_INSPECT_COMMAND,
        {
          request: {
            operation_id: operationId,
          } satisfies VcpCliKnowledgeInspectRequest,
        },
      );
      if (
        !isCurrentView(view) ||
        mutation !== knowledgeMutationGeneration ||
        pendingKnowledgeInspectOperationId.value !== operationId
      ) {
        return;
      }
      if (response.operation_id !== operationId) {
        throw new Error("知识导入 operation_id 回执不匹配");
      }
      pendingKnowledgeInspectOperationId.value = null;
      if (response.status === "cancelled") {
        knowledgeMutationNotice.value = "已取消文件选择，未创建授权。";
        return;
      }
      knowledgeImportCandidate.value = response.candidate;
      ...
      knowledgeMutationError.value = {
        code: "knowledge_import_error",
        message: describeError(error),
      };
      // Preserve the operation id so an ambiguous picker/inspect IPC can replay safely.
    } finally {
      if (mutation === knowledgeMutationGeneration) {
        knowledgeMutationBusy.value = false;
      }
    }
  }

  async function commitKnowledgeImport(): Promise<void> {
    ...
    const operationId =
      pendingKnowledgeCommitOperationId.value ??
      newOperationId("knowledge-commit");
    pendingKnowledgeCommitOperationId.value = operationId;
    knowledgeMutationBusy.value = true;
    knowledgeMutationError.value = null;
    knowledgeMutationNotice.value = "";
    try {
      const response = await invoke<VcpCliKnowledgeCommitResponse>(
        VCP_CLI_KNOWLEDGE_IMPORT_COMMIT_COMMAND,
        {
          request: {
            ...
            candidate_sha256: candidate.candidate_sha256,
            expected_catalog_generation: candidate.catalog_generation,
          } satisfies VcpCliKnowledgeCommitRequest,
        },
      );
      if (
        !isCurrentView(view) ||
        mutation !== knowledgeMutationGeneration ||
        knowledgeImportCandidate.value?.token !== candidate.token
      ) {
        return;
      }
      if (response.operation_id !== operationId) {
      ...
      knowledgeGenerationFloor = Math.max(
        knowledgeGenerationFloor,
        response.catalog_generation,
      );
      knowledgeImportCandidate.value = null;
      pendingKnowledgeCommitOperationId.value = null;
      pendingKnowledgeDiscardOperationId.value = null;
      knowledgeMutationNotice.value = "本机知识授权已提交，正在重读 catalog。";
  ...
  async function discardKnowledgeImport(): Promise<void> {
    const candidate = knowledgeImportCandidate.value;
    if (
      !viewVisible.value ||
      !candidate ||
    ...
    const operationId =
      pendingKnowledgeDiscardOperationId.value ??
      newOperationId("knowledge-discard");
    pendingKnowledgeDiscardOperationId.value = operationId;
    knowledgeMutationBusy.value = true;
    knowledgeMutationError.value = null;
    knowledgeMutationNotice.value = "";
    try {
      const response = await invoke<VcpCliKnowledgeDiscardResponse>(
        VCP_CLI_KNOWLEDGE_IMPORT_DISCARD_COMMAND,
        {
          request: {
            operation_id: operationId,
            token: candidate.token,
          } satisfies VcpCliKnowledgeDiscardRequest,
        },
      );
      if (
        !isCurrentView(view) ||
      ...
      if (response.operation_id !== operationId) {
        throw new Error("知识候选清理 operation_id 回执不匹配");
      }
      knowledgeImportCandidate.value = null;
      pendingKnowledgeCommitOperationId.value = null;
      pendingKnowledgeDiscardOperationId.value = null;
      knowledgeMutationNotice.value = "导入候选已删除，未创建授权。";
      await loadKnowledgeCatalog();
    } catch (error) {
      if (
        isCurrentView(view) &&
  ...
  async function revokeKnowledgeSource(
    source: VcpCliKnowledgeSource,
  ): Promise<void> {
    const catalog = knowledgeCatalog.value;
    const existing = pendingKnowledgeRevoke.value;
    if (
      !viewVisible.value ||
      !catalog ||
      knowledgeMutationBusy.value ||
      ...
      pendingKnowledgeCommitOperationId.value ||
      pendingKnowledgeDiscardOperationId.value ||
      (existing && existing.sourceId !== source.source_id)
    ) {
      return;
    }
    const mutation = ++knowledgeMutationGeneration;
    const view = viewGeneration.value;
    const operation =
      existing?.sourceId === source.source_id &&
      existing.expectedCatalogGeneration === catalog.catalog_generation
        ? existing
        : {
            operationId: newOperationId("knowledge-revoke"),
            sourceId: source.source_id,
            expectedCatalogGeneration: catalog.catalog_generation,
          };
    pendingKnowledgeRevoke.value = operation;
    knowledgeMutationBusy.value = true;
    knowledgeMutationError.value = null;
    knowledgeMutationNotice.value = "";
    try {
      const response = await invoke<VcpCliKnowledgeRevokeResponse>(
        VCP_CLI_KNOWLEDGE_REVOKE_COMMAND,
        {
          request: {
            operation_id: operation.operationId,
            source_id: operation.sourceId,
            expected_catalog_generation: operation.expectedCatalogGeneration,
          } satisfies VcpCliKnowledgeRevokeRequest,
        },
      );
      if (
        !isCurrentView(view) ||
        mutation !== knowledgeMutationGeneration ||
        pendingKnowledgeRevoke.value?.operationId !== operation.operationId
      ) {
        return;
      }
      if (
        response.operation_id !== operation.operationId ||
        response.source_id !== operation.sourceId
      ) {
        throw new Error("知识撤权 operation_id/source_id 回执不匹配");
      }
      knowledgeGenerationFloor = Math.max(
        knowledgeGenerationFloor,
        response.catalog_generation,
      ...
      pendingKnowledgeRevoke.value = null;
      knowledgeMutationNotice.value =
        response.deletion_state === "pending_holds"
          ? "已从未来召回中撤权；运行准入持有释放后删除本机对象。"
          : "已撤权并删除本机知识对象。";
      await loadKnowledgeCatalog();
    } catch (error) {
      if (
